...
<td>Directly emits observed feature values that become final
<code>X</code>; the target is then generated by the conditional
observed-<code>X</code> head.</td>
</tr>
<tr>
...
<td>Section 6.2.</td>
<td>Emits categorical feature labels that populate final observed
features.</td>
</tr>
</tbody>
</table>
...
<td>Defines the emission boundary from latent state to observed feature
values.</td>
</tr>
<tr>
...
observed <code>X</code> -&gt; conditional target head -&gt; final
<code>y</code>.<br></p>
...
-&gt; feature converter extraction -&gt; observed <code>X</code> -&gt;
...
rewritten latent slices -&gt; assigned feature outputs -&gt; observed
...
prior, this node pipeline emits observed features only. The target is then
generated from the postprocessed observed feature matrix by a separately
sampled conditional head.</p>
...
observable value for the feature key.</li>
</ol>
...
translate latent causal state into the tabular features a
...
separate observed-<code>X</code> conditional head applied after feature
...
slots and become emitted <code>X</code>; the conditional target head
then maps observed <code>X</code> to <code>y</code>; <span
...
numeric feature value in <code>X</code>; <span
