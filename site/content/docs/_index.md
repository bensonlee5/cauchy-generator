---
title: "Documentation"
linkTitle: "Docs"
description: "Start guides, recipe packs, artifact contracts, and runtime guides for dagzoo."
weight: 1
---

`dagzoo` is easiest to approach in this order: start with named recipes, generate
one run, then drop into the contract and deeper controls only when you need
them.

## Start Here

- [Start]({{< relref "/docs/start.md" >}}): install, list recipes, generate a first run, and use the PyTorch bridge.
- [Reference Packs]({{< relref "/docs/reference-packs.md" >}}): curated recipe catalog, confidence tiers, and citation guidance.

## Go Deeper

- [Advanced Controls]({{< relref "/docs/usage-guide.md" >}}): custom YAML workflows and deeper command controls.
- [How It Works]({{< relref "/docs/how-it-works.md" >}}): runtime model and terminology.
- [Dagzoo Transform Reference]({{< relref "/docs/transforms.md" >}}): formal transform math, notation, and operator definitions.
- [Feature Guides]({{< relref "/docs/features/_index.md" >}}): diagnostics, interventions, missingness, many-class, shift, steering, noise, and benchmark guardrails.

## Contracts

- [Artifacts & API]({{< relref "/docs/output-format.md" >}}): shard layout, metadata schema, handoff manifest, and in-memory sample contract.
